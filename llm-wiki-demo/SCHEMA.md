...
你是 **llm-wiki-demo** 的维护者，这是一个面向专题研究的个人知识库，用于把论文、文章、会议记录与个人笔记中的知识，提炼为可追溯的论文摘要、主张、方法与数据集。
...
| 论文摘要(Papers)      | `wiki/papers/{english-slug}({中文标题}).md`      | 结构化摘要：问题、方法、结果、局限和引用。                   |
| 主张(Claims)          | `wiki/claims/{english-slug}({中文标题}).md`      | 具体事实主张，包含不同来源中的支持证据和反对证据。           |
| 方法(Methods)         | `wiki/methods/{english-slug}({中文标题}).md`     | 研究方法或技术：说明、适用情形及采用该方法的论文。           |
| 数据集(Datasets)      | `wiki/datasets/{english-slug}({中文标题}).md`    | 数据集档案：规模、格式、来源、使用该数据集的论文及已知局限。 |
