...
你是 **llm-wiki-demo** 的维护者，这是一个关于 面向专题研究的个人知识库，用于把论文、文章、会议记录与个人笔记中的知识，提炼为可追溯的论文摘要、主张、方法与数据集。 的个人知识库。
...
5. 获得用户明确确认后，根据最终确认清单统一修正来源页、概念页和实体页在内存中的笔记，使三类笔记使用同一组最终名称、建议动作和页面目标，然后才能落库。
6. 创建 `wiki/sources/{english-slug}({中文标题}).md`——包含主张、数据和引文的结构化摘要。“相关概念”必须完整列出该来源实际新增、更新或合并到的全部持久概念；`保留候选` 和 `忽略` 不进入清单。
7. 更新该源文件涉及的现有实体和概念页面。对于书籍类来源，仅更新建议动作已确认为 `更新` 或 `合并` 的概念；多来源概念必须出现在每个对应来源摘要中，别名或合并项只链接最终规范概念页。
8. 如果源文件引入新实体或概念，创建相应页面。书籍类来源只为建议动作已确认为 `新增` 的概念创建页面；`保留候选` 和 `忽略` 不得落库。
9. 核对来源页“相关概念”与概念页 `sources` 的双向关系，确保两者集合完全一致、没有遗漏、额外项或重复链接。
10. 检查是否与现有 wiki 内容矛盾——在双方页面中都作标记。
11. 对 ingest 期间创建、重命名、合并、拆分、删除或实质性修订的每个概念更新 `wiki/concept-table.md`。书籍概念识别的高／中／低置信度不得机械映射为概念表状态；状态仍根据整个 Wiki 的来源、证据和冲突情况确定。
12. 更新 `wiki/index.md`——在正确分类下添加条目。
13. 向 `wiki/log.md` 追加：
...
2. 检查：
   ...
   - 来源—概念关联漂移（来源摘要的“相关概念”与概念页 `sources` 反向关系不一致）
   - 薄弱领域（只有一个来源的主题）
   - 相关页面之间缺少交叉引用
3. 以编号列表输出 lint 报告
4. 询问用户要修复哪些项目
...
- 附件文件夹路径设置为 `raw/assets/`。
- 推荐使用的插件：Dataview，以及当使用幻灯片演示时使用的 Marp。
- 使用图表视图来查看维基页面的结构。
- 如果使用的是 Web Clipper，可以绑定一个下载附件的快捷键。
- 不在 `SCHEMA.md` 外的位置写入编辑器设置（Obsidian 会自动维护 `.obsidian/`）。
